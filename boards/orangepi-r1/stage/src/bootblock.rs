//! Bootblock fixed-flow adapter for Orange Pi R1.
...
impl HardwareInit for BootblockDevices {
    fn early_clocks(&mut self, _ctx: &mut InitContext<'_>) -> Result<(), ServiceError> {
        let mut ccu = SunxiH3Ccu::new(&CCU_CONFIG).map_err(device_error_to_service_error)?;
    ...
    fn dram(&mut self, _ctx: &mut InitContext<'_>) -> Result<(), ServiceError> {
        let mut dramc = SunxiH3Dramc::new(&DRAMC_CONFIG).map_err(device_error_to_service_error)?;
