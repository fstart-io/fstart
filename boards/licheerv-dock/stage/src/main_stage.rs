//! Main-stage fixed-flow adapter for Lichee RV Dock.
...
impl StaticBoard for MainBoard {
    ...
    fn boot_payload(self) -> ! {
        if !self.boot.firmware_loaded || !self.boot.kernel_loaded || !self.boot.dtb_loaded {
            fstart_log::error!("Linux handoff requested before SBI/kernel/DTB load");
            Self::halt();
        }

        fstart_log::info!(
            "booting SBI at {:#x}, kernel at {:#x}, dtb at {:#x}, dram={} MiB",
