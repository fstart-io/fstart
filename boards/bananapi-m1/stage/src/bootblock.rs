//! Bootblock fixed-flow adapter for Banana Pi M1.

use fstart_board_bananapi_m1_facts as facts;
use fstart_driver_ns16550::Ns16550;
use fstart_driver_sunxi_a20_dramc::SunxiA20Dramc;
use fstart_driver_sunxi_ccu::SunxiA20Ccu;
...
pub struct BootblockDevices {
    ccu: Option<SunxiA20Ccu>,
    console: StaticConsole<Ns16550>,
    dramc: Option<SunxiA20Dramc>,
...
impl HardwareInit for BootblockDevices {
    fn early_clocks(&mut self, _ctx: &mut InitContext<'_>) -> Result<(), ServiceError> {
        let mut ccu = SunxiA20Ccu::new(&CCU_CONFIG).map_err(device_error_to_service_error)?;
    ...
    fn dram(&mut self, _ctx: &mut InitContext<'_>) -> Result<(), ServiceError> {
        let mut dramc = SunxiA20Dramc::new(&DRAMC_CONFIG).map_err(device_error_to_service_error)?;
