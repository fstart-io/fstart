//! Main-stage fixed-flow adapter for Orange Pi PC2.
...
impl BlockLinuxBoot {
    ...
    fn load_linux(&mut self, mmc: &SunxiMmc) -> Result<(), ServiceError> {
        self.load_file(mmc, FileType::Firmware)?;
    ...
    fn prepare_fdt(&self, dram_size: u64) -> Result<(), ServiceError> {
        #[cfg(feature = "fdt")]
...
impl StaticBoard for MainBoard {
    ...
    fn finalize_handoff(&mut self, _ctx: &mut InitContext<'_>) -> Result<(), ServiceError> {
        self.boot.prepare_fdt(self.dram_size)
    }

    fn boot_payload(self) -> ! {
        if !self.boot.firmware_loaded || !self.boot.kernel_loaded || !self.boot.dtb_loaded {
            fstart_log::error!("Linux handoff requested before BL31/kernel/DTB load");
            Self::halt();
        }

        fstart_log::info!(
            "booting BL31 at {:#x}, kernel at {:#x}, dtb at {:#x}, dram={} MiB",
        ...
        let params = fstart_services::boot::BootLinuxParams {
            ...
            fw_addr: facts::FIRMWARE_LOAD_ADDR,
            rsdp_addr: 0,
