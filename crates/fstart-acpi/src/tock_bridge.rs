...
impl TockAcpiField {
    ...
    /// Create from a pre-built list of field entries.
    ///
...
impl Aml for TockAcpiField {
    fn to_aml_bytes(&self, sink: &mut dyn AmlSink) {
        let reg_bytes = (self.reg_bits / 8) as u64;

        // OperationRegion
        let offset_val = self.offset;
        let length_val = reg_bytes;
        ...
        // Field
        let access = access_type_for_width(self.reg_bits);
        let field = Field::new(
            Path::new(self.region_name),
            access,
...
pub fn build_multi_register_field(
    ...
    TockAcpiField::from_entries(region_name, space, base_offset, total_bits, entries)
}
