//! Intel ICH9 / ICH9-M southbridge driver.
//!
//! The initial target is the Lenovo ThinkPad X200 (GM45 + ICH9-M). The
//! reusable pre-console path opens the southbridge LPC/GPIO decode needed by
//! board hooks. X200-specific EC/PMH7 policy lives in the
//! `fstart-mainboard-lenovo-x200` crate.
...
/// ICH9/ICH9-M PCI config and RCBA constants.
pub mod ich9 {
    ...
    /// ICH9-M LPC IDs from coreboot `i82801ix.h::lpc_is_mobile()`.
    pub const DID_ICH9M_LPC_AMT: u16 = 0x2917;
    pub const DID_ICH9M_LPC_BASE: u16 = 0x2919;
    pub const DID_ICH9M_SATA_IDE_P01: u16 = 0x2928;
    pub const DID_ICH9M_SATA_AHCI_P0145: u16 = 0x2929;

    pub const FD_BIOS_MUST_WRITE: u32 = 1 << 0;
    ...
    /// UHCI #6 disable bit, present on ICH9 (`i82801ix.h:FD_U6D`).
    pub const FD_U6D: u32 = 1 << 7;
...
pci_type0_config! {
    /// ICH9 LPC bridge PCI configuration space.
    pub struct Ich9LpcPciConfig {
...
register_structs! {
    /// ICH9 Root Complex Base Address MMIO register block.
    RcbaRegs {
        ...
        (0x3414 => pub buc: MmioReadWrite<u32>),
...
const GPE0_STS_ICH9: u16 = 0x20;
const GPE0_EN_ICH9: u16 = 0x28;
...
/// ICH9-M PATA/IDE controller configuration.
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
...
/// Board-provided I/O trap programming for ICH9 RCBA.
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct Ich9IoTrapConfig {
...
/// Board-provided late RCBA interrupt routing for ICH9.
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
pub struct Ich9LateRcbaConfig {
    ...
    #[serde(default)]
    pub iotr3: Option<Ich9IoTrapConfig>,
}

/// PIRQ route value for ICH9 PIRQ[A-H]_ROUT registers.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum PirqRoute {
    /// Route to ISA IRQ 3.
    Irq3,
...
/// GPI routing selector for GPIO0..15.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
pub enum GpiRoute {
    /// Do not route this GPI to SMI or SCI.
    #[default]
    None,
    /// Route this GPI to SMI.
    Smi,
    /// Route this GPI to SCI.
    Sci,
}
...
/// ICH9 southbridge configuration.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct IntelIch9Config {
    /// Root Complex Base Address register value.
    pub rcba: u64,
    /// Northbridge DMIBAR base address, used as the RCBA upstream RCRB target.
    pub dmibar: u64,
    /// PIRQ routing (A..H).
    pub pirq_routing: [PirqRoute; 8],
    /// GPE0 enable bits (ICH9 low dword at PMBASE+0x28).
    pub gpe0_en: u32,
    /// GPI routing selectors for GPIO0..15.
    #[serde(default)]
    ...
    /// Disable the second SATA function. ICH9-M boards commonly leave it hidden.
    #[serde(default = "default_true")]
    pub disable_sata2: bool,
    /// Disable the desktop thermal-throttle function. ICH9-M does not expose it.
    #[serde(default = "default_true")]
    pub disable_thermal: bool,
    /// Optional board-provided late RCBA interrupt/trap routing.
    #[serde(default)]
    pub late_rcba: Option<Ich9LateRcbaConfig>,
}
...
/// Intel ICH9 southbridge driver.
pub struct IntelIch9 {
    config: &'static IntelIch9Config,
    smbus: Option<I801SmBus>,
    pm: PmIo,
}

// SAFETY: firmware performs chipset init on the BSP before concurrency exists.
unsafe impl Send for IntelIch9 {}
// SAFETY: the struct contains only config, optional bus state, and fixed I/O bases.
unsafe impl Sync for IntelIch9 {}

impl IntelIch9 {
    fn lpc(&self) -> ecam::EcamDevice {
        ecam::EcamDevice::new(0, ich9::LPC_DEV, ich9::LPC_FUNC)
    }

    fn lpc_regs(&self) -> &'static Ich9LpcPciConfig {
        let lpc = self.lpc();

        // SAFETY: the ICH9 LPC bridge is a fixed chipset device at 00:1f.0,
        ...
        unsafe { lpc.regs::<Ich9LpcPciConfig>() }
    }
    ...
    fn enable_spi_prefetching_and_caching(&self) {
        let lpc = self.lpc();
        // Match coreboot i82801ix bootblock: enable SPI prefetch/cache before
    ...
    fn program_fixed_bars(&self) {
        ...
        self.lpc()
            .write32(ich9::RCBA, (self.config.rcba as u32 & 0xffff_c000) | 1);
        lpc.pmbase.set((ich9::DEFAULT_PMBASE as u32) | 1);
        lpc.acpi_cntl.set(0x80);
        lpc.gpio_base.set(ich9::DEFAULT_GPIOBASE as u32);
    ...
    fn setup_gpios(&self) {
        let gpio = IchGpio::new(ich9::DEFAULT_GPIOBASE);
        gpio.setup(&self.config.gpio);
    }

    fn enable_smbus(&mut self) {
        let smbus_pci = ecam::EcamDevice::new(0, ich9::SMBUS_DEV, ich9::SMBUS_FUNC);
        smbus_pci.and16(0x80, !((1 << 8) | (1 << 10) | (1 << 12) | (1 << 14)));
        let smbus =
            I801SmBus::enable_on_i801(0, ich9::SMBUS_DEV, ich9::SMBUS_FUNC, self.config.smbus_base);
    ...
    fn setup_dmi(&self) {
        ...
        // Mobile ICH9-M/HX path: enable DMI mobile power savings, then
    ...
    fn poll_vc1(&self) {
        ...
        if timeout == 0 {
            fstart_log::error!("intel-ich9: VC1 negotiation timeout");
        }
        ...
        if timeout == 0 {
            fstart_log::error!("intel-ich9: VC1 arbitration-table update timeout");
        }
    }
    ...
    fn configure_power_options(&self) {
        ...
        self.pm().write32(GPE0_STS_ICH9, 0xffff_ffff);
        self.pm().write32(GPE0_EN_ICH9, self.config.gpe0_en);
        self.pm()
        ...
        unsafe {
            ...
            // Coreboot reads NMI state from 0x74 because CMOS index port 0x70
            // is write-only on this chipset, then writes the NMI-disabled
            // value back to 0x70.
    ...
    fn rtc_init_status(&self) {
        ...
        if lpc.gen_pmcon_3.is_set(GEN_PMCON_3_REG::RTC_BATTERY_DEAD) {
            ...
            fstart_log::info!("intel-ich9: RTC battery-dead flag was set");
        }
    }
    ...
    fn ramstage_lpc_init(&self) {
        ...
        self.disable_acpi_pm1_for_os_handoff();
    }
    ...
    fn usb_fd_bit(dev: u8, func: u8) -> u32 {
        match (dev, func) {
            (ich9::UHCI1_DEV, 0) => ich9::FD_U1D,
            (ich9::UHCI1_DEV, 1) => ich9::FD_U2D,
            (ich9::UHCI1_DEV, 2) => ich9::FD_U3D,
            (ich9::EHCI1_DEV, ich9::EHCI1_FUNC) => ich9::FD_EHCI1D,
            (ich9::UHCI2_DEV, 0) => ich9::FD_U4D,
            (ich9::UHCI2_DEV, 1) => ich9::FD_U5D,
            (ich9::UHCI2_DEV, 2) => ich9::FD_U6D,
            (ich9::EHCI2_DEV, ich9::EHCI2_FUNC) => ich9::FD_EHCI2D,
            _ => 0,
        }
    }
    ...
    fn early_chipset_settings(&self) {
        ...
        // Coreboot `i82801ix_early_settings()` sets RCBA_FD bit 0 as a
        // mandatory BIOS write while preserving board-selected disable bits.
        rcba.regs()
            .fd
            .set(rcba.regs().fd.get() | ich9::FD_BIOS_MUST_WRITE);
        ...
        // Coreboot intentionally leaves CIR5 bit 0 commented out per the
        // ICH9 specification update; do not set it here.
    ...
    fn configure_gpi_routing(&self) {
        ...
        for (idx, route) in self.config.gpi_routing.iter().enumerate() {
            value |= ((route.bits() as u32) & 0x03) << (idx * 2);
        }
        self.lpc_regs().gpio_rout.set(value);
    }

    fn sata_indexed_write32(&self, idx: u8, val: u32) {
        let sata = ecam::EcamDevice::new(0, ich9::SATA_DEV, ich9::SATA_FUNC);
        sata.write8(ich9::SATA_SIDX, idx);
        sata.write32(ich9::SATA_SDAT, val);
    }

    fn sata_indexed_rmw32(&self, idx: u8, clear: u32, set: u32) {
        let sata = ecam::EcamDevice::new(0, ich9::SATA_DEV, ich9::SATA_FUNC);
        sata.write8(ich9::SATA_SIDX, idx);
        let val = sata.read32(ich9::SATA_SDAT);
        sata.write32(ich9::SATA_SDAT, (val & !clear) | set);
    }

    fn sata_enable_ahci_mmap(&self, sata: &SataConfig, is_mobile: bool) {
        // ICH9-M AHCI exposes ports 0, 1, 4, and 5; coreboot skips 2/3 when
        // touching per-port ABAR registers.
        let port_mask = if is_mobile { 0x33 } else { 0x3f };
    ...
    fn sata_program_indexed(&self, is_mobile: bool) {
        ...
        let sata = ecam::EcamDevice::new(0, ich9::SATA_DEV, ich9::SATA_FUNC);
        sata.write8(ich9::SATA_SIDX, 0x40);
        sata.write8(ich9::SATA_SDAT + 2, 0x22);
        sata.write8(ich9::SATA_SIDX, 0x78);
        sata.write8(ich9::SATA_SDAT + 2, 0x22);
        if !is_mobile {
    ...
    fn sata_init(&self, config: &SataConfig) {
        ...
        let sata_devid = sata.read16(ich9::PCI_DEVICE_ID);
        let is_mobile = matches!(
            sata_devid,
            ich9::DID_ICH9M_SATA_IDE_P01 | ich9::DID_ICH9M_SATA_AHCI_P0145
        );
        let port_mask = if is_mobile { 0x33 } else { 0x3f };
        ...
        match config.mode {
            SataMode::Ahci => {
                let map = 0x60 | (u16::from((config.ports ^ 0x3f) & 0x3f) << 8);
                sata.write16(ich9::SATA_MAP, map);
                sata.write32(ich9::PCI_BAR5, SATA_ABAR_BASE as u32);
            }
            SataMode::Ide => {
                sata.write16(ich9::SATA_MAP, u16::from((config.ports ^ 0x3f) & 0x3f) << 8);
                // Prog IF is modeled read-only in the generic header overlay,
                // so keep this exact raw programming write.
                sata.write8(ich9::PCI_CLASS_PROG, 0x8f);
                sata.write32(ich9::PCI_BAR5, 0);
            }
        }
        sata.write16(ich9::SATA_IDE_TIM_PRI, 1 << 15);
        sata.write16(ich9::SATA_IDE_TIM_SEC, 1 << 15);
        let pcs_ports = if matches!(config.mode, SataMode::Ahci) {
            0x3f
        } else {
            u16::from(ports)
        };
        let pcs = (sata.read16(ich9::SATA_PCS) & !0x3f) | (1 << 15) | pcs_ports;
        sata.write16(ich9::SATA_PCS, pcs);
        ...
        if config.clock_request {
            ...
            if unsafe { fstart_pio::inb(ich9::DEFAULT_GPIOBASE + 0x30) } & (1 << (35 - 32)) == 0 {
                sclkcg |= 1 << 30;
            }
        }
        sata.write32(ich9::SATA_CLK, sclkcg);
        if config.traffic_monitor && ((self.lpc().read8(ich9::CXSTATE_CNF) >> 3) & 3) == 3 {
        ...
        fstart_log::info!("intel-ich9: SATA init complete ports={:#x}", ports as u32);
    }
    ...
    fn ehci_init_controller(&self, dev: u8, func: u8) {
        ...
        // Match coreboot's current ICH9 EHCI init: do not assign a temporary
        ...
        // Coreboot `i82801ix_ehci_init()`: set EHCIIR bits 29 and 17 and
        // program bits [3:2] to 10b, preserving the rest of the register.
        ehci.modify32(
            ich9::EHCI_INTEL_FCREG,
            !(3 << 2),
            (1 << 29) | (1 << 17) | (2 << 2),
        );
    }
    ...
    fn usb_init(&self) {
        ...
        if usb.ehci[0] {
            self.ehci_init_controller(ich9::EHCI1_DEV, ich9::EHCI1_FUNC);
        }
        if usb.ehci[1] {
            self.ehci_init_controller(ich9::EHCI2_DEV, ich9::EHCI2_FUNC);
        }
        const UHCI: [(u8, u8); 6] = [
            (0x1d, 0),
            (0x1d, 1),
        ...
        fstart_log::info!("intel-ich9: USB init complete");
    }

    fn hda_init(&self, config: &HdaConfig) {
        let hda = ecam::EcamDevice::new(0, ich9::HDA_DEV, ich9::HDA_FUNC);
        if hda.read16(0) == 0xffff {
            fstart_log::info!("intel-ich9: HDA device not present");
            return;
        }

        // Coreboot i82801ix/azalia.c: ESD/link/VC setup before codec reset.
        ...
        if bar0 == 0 {
            fstart_log::error!("intel-ich9: HDA BAR0 is unassigned");
            return;
        }
        hda_regs
            .command
        ...
        if codec_mask != 0 {
            ...
            fstart_log::info!(
                "intel-ich9: HDA codec_mask={:#x}, programmed={} tables",
                codec_mask as u32,
                programmed as u32,
            );
        }
    ...
    fn pcie_port_cir_init(&self) {
        for func in 0u8..6 {
            let port = ecam::EcamDevice::new(0, ich9::PCIE_DEV, func);
            if port.read16(0) == 0xffff {
                continue;
            }

            // Match coreboot i82801ix/pcie.c CIR programming: set CIR 0x300
            // bit 21 and write 0x40 to CIR 0x324 without touching other hidden
            // config bits.
            port.or32(ich9::D28FX_CIR_300, 1 << 21);
            port.write8(ich9::D28FX_CIR_324, 0x40);
        }
    }
    ...
    fn pcie_init(&self) {
        ...
        for func in 0u8..6 {
            ...
            port.or32(ich9::D28FX_ASPM_MOBILE, 1);
            if (port.read32(ich9::D28FX_LCTL) & 3) == 3 {
                port.or32(ich9::D28FX_ASPM_MOBILE, 1 << 1);
            }
            ...
            port.or32(ich9::D28FX_IOXAPIC, 1 << 7);
            port.or8(ich9::D28FX_BBCLKG, 0x0f);
            port.write32(
                ich9::D28FX_VC0RCTL,
                (port.read32(ich9::D28FX_VC0RCTL) & !0xff) | 1,
            );
            port.or32(ich9::D28FX_CTTOMASK, 1 << 14);
            port.write32(ich9::D28FX_CEMASK, port.read32(ich9::D28FX_CEMASK));
            port.write16(ich9::PCI_STATUS, port.read16(ich9::PCI_STATUS));
            port.write16(ich9::PCI_SEC_STATUS, port.read16(ich9::PCI_SEC_STATUS));
        }
        let fd = self.rcba().regs().fd.get() | self.function_disable_mask();
        for func in (0usize..6).rev() {
            if (fd & Self::pcie_fd_bit(func)) == 0 {
                break;
            }
            let port = ecam::EcamDevice::new(0, ich9::PCIE_DEV, func as u8);
            if port.read16(0) != 0xffff {
                port.or32(ich9::D28FX_CIR_300, 0x3 << 16);
            }
        }
        let rcba = self.rcba();
        ...
        self.pcie_aspm_lock();
        fstart_log::info!("intel-ich9: PCIe root port init complete");
    }
    ...
    fn pcie_slot_config(&self) {
        ...
        for func in 0usize..6 {
            ...
            if self.config.pcie_slots[func] {
                port.or32(ich9::D28FX_XCAP, ich9::D28FX_XCAP_SLOT);
                let limit = self.config.pcie_power_limits[func];
                let mut slcap = port.read32(ich9::D28FX_SLCAP);
                slcap &= !(0x1fff << ich9::D28_SLCAP_SLOTNUM_SHIFT);
                slcap |= slot_number << ich9::D28_SLCAP_SLOTNUM_SHIFT;
                slcap &= !(0x03 << ich9::D28_SLCAP_SCALE_SHIFT);
                slcap |= (u32::from(limit.scale) & 0x03) << ich9::D28_SLCAP_SCALE_SHIFT;
                slcap &= !(0xff << ich9::D28_SLCAP_POWER_SHIFT);
                slcap |= u32::from(limit.value) << ich9::D28_SLCAP_POWER_SHIFT;
                port.write32(ich9::D28FX_SLCAP, slcap);
                slot_number += 1;
            } else {
                port.and32(ich9::D28FX_XCAP, !ich9::D28FX_XCAP_SLOT);
            }
        }
    }
    ...
    fn pcie_aspm_lock(&self) {
        for func in 0u8..6 {
            let port = ecam::EcamDevice::new(0, ich9::PCIE_DEV, func);
            if port.read16(0) != 0xffff {
                port.write32(ich9::D28FX_LCAP, port.read32(ich9::D28FX_LCAP));
            }
        }
    }

    fn ide_init(&self, config: &IdeConfig) {
        let ide = ecam::EcamDevice::new(0, ich9::IDE_DEV, ich9::IDE_FUNC);
        ...
        ide.write8(ich9::PCI_CLASS_PROG, 0x8a);

        let timing_base = ich9::IDE_SITRE
            | ich9::IDE_ISP_3_CLOCKS
            | ich9::IDE_RCT_1_CLOCKS
            | ich9::IDE_IE0
            | ich9::IDE_TIME0;
        let primary_timing = (ide.read16(ich9::IDE_TIM_PRI) & !ich9::IDE_DECODE_ENABLE)
            | timing_base
            | if config.enable_primary {
                ich9::IDE_DECODE_ENABLE
            } else {
                0
            };
        let secondary_timing = (ide.read16(ich9::IDE_TIM_SEC) & !ich9::IDE_DECODE_ENABLE)
            | timing_base
        ...
        ide.write16(ich9::IDE_TIM_PRI, primary_timing);
        ide.write16(ich9::IDE_TIM_SEC, secondary_timing);

        let mut ide_config = 0u32;
        if config.enable_primary {
            ide_config |= ich9::FAST_PCB0 | ich9::PCB0 | ich9::FAST_PCB1 | ich9::PCB1;
        }
        if config.enable_secondary {
            ide_config |= ich9::FAST_SCB0 | ich9::SCB0 | ich9::FAST_SCB1 | ich9::SCB1;
        }
        ide.write32(ich9::IDE_CONFIG, ide_config);
        ide_regs.interrupt_line.set(0xff);
    }

    fn pci_bridge_init(&self) {
        let bridge = ecam::EcamDevice::new(0, ich9::PCI_BRIDGE_DEV, ich9::PCI_BRIDGE_FUNC);
        if bridge.read16(0) == 0xffff {
            return;
        }
        Self::type1_regs(bridge).interrupt_line.set(0xff);
        bridge.and8_or8(ich9::D30F0_SMLT, 0x07, 0x04 << 3);
        bridge.write16(ich9::PCI_STATUS, bridge.read16(ich9::PCI_STATUS));
        bridge.write16(ich9::PCI_SEC_STATUS, bridge.read16(ich9::PCI_SEC_STATUS));
    }

    fn isa_dma_init(&self) {
        #[cfg(target_arch = "x86_64")]
        fstart_arch_x86::legacy_pc::isa_dma_init();
    }

    fn i8259_init(&self) {
        #[cfg(target_arch = "x86_64")]
        fstart_arch_x86::legacy_pc::i8259_init();
    }

    fn disable_acpi_pm1_for_os_handoff(&self) {
        // Match coreboot's cold-boot handoff: install the SMI handler but leave
        // SCI_EN clear so OSPM transitions to ACPI mode through FADT SMI_CMD.
        // S3 resume will need to send the ACPI_ENABLE command once implemented.
        let pm1 =
            (self.pm().read32(pmio::PM1_CNT) & !(pmio::SLP_TYP_MASK | pmio::SCI_EN)) | pmio::BM_RLD;
    ...
    fn apply_function_disable(&self) {
        let rcba = self.rcba();
        let fd = rcba.regs().fd.get() | self.function_disable_mask() | ich9::FD_BIOS_MUST_WRITE;
        rcba.regs().fd.set(fd);
        if self.config.disable_lan {
            rcba.regs().buc.set(rcba.regs().buc.get() | (1 << 5));
        }
        fstart_log::info!("intel-ich9: function disable mask={:#x}", fd);
    }

    fn clear_disabled_device_commands(&self) {
        if self.config.disable_lan {
            Self::clear_pci_command(ich9::LAN_DEV, ich9::LAN_FUNC);
        }
        if self.config.hda.is_none() {
            Self::clear_pci_command(ich9::HDA_DEV, ich9::HDA_FUNC);
        }
        if self.config.sata.is_none() {
            Self::clear_pci_command(ich9::SATA_DEV, ich9::SATA_FUNC);
        }
        if self.config.disable_sata2 {
            Self::clear_pci_command(ich9::SATA2_DEV, ich9::SATA2_FUNC);
        }
        if self.config.disable_thermal {
            Self::clear_pci_command(ich9::THERMAL_DEV, ich9::THERMAL_FUNC);
        }
        if let Some(usb) = self.config.usb {
            ...
            if !usb.ehci[0] {
                Self::clear_pci_command(ich9::EHCI1_DEV, ich9::EHCI1_FUNC);
            }
            if !usb.ehci[1] {
                Self::clear_pci_command(ich9::EHCI2_DEV, ich9::EHCI2_FUNC);
            }
        }
        for (idx, enabled) in self.config.pcie_ports.iter().enumerate() {
            if !*enabled {
                Self::clear_pci_command(ich9::PCIE_DEV, idx as u8);
            }
        }
    }
    ...
    fn write_pirq_routes(&self) {
        ...
        let pirq_low = u32::from_le_bytes([
            self.config.pirq_routing[0].bits(),
            self.config.pirq_routing[1].bits(),
            self.config.pirq_routing[2].bits(),
            self.config.pirq_routing[3].bits(),
        ]);
        let pirq_high = u32::from_le_bytes([
            self.config.pirq_routing[4].bits(),
            self.config.pirq_routing[5].bits(),
            self.config.pirq_routing[6].bits(),
            self.config.pirq_routing[7].bits(),
        ]);
    ...
    fn configure_late_rcba(&self, config: &Ich9LateRcbaConfig) {
        let rcba = self.rcba();
...
impl Device for IntelIch9 {
    const NAME: &'static str = "intel-ich9";
    const COMPATIBLE: &'static [&'static str] = &["intel,ich9", "intel,ich9m", "intel,82801ix"];
    type Config = IntelIch9Config;

    fn new(config: &'static IntelIch9Config) -> Result<Self, DeviceError> {
        if config
            .lpc_decode
            .generic_io
            .iter()
        ...
        Ok(Self {
            config,
            smbus: None,
            pm: PmIo::new(ich9::DEFAULT_PMBASE),
        })
    }
...
impl PreConsoleInit for IntelIch9 {
...
impl EarlyInit for IntelIch9 {
    fn early_init(&mut self) -> Result<(), ServiceError> {
        ...
        fstart_log::info!("intel-ich9: early init complete");
        Ok(())
    }
}

impl PostDramInit for IntelIch9 {
    fn post_dram_init(&mut self) -> Result<(), ServiceError> {
        ...
        fstart_log::info!("intel-ich9: ramstage init complete");
        Ok(())
    }
}

impl FinalizeInit for IntelIch9 {
...
impl Southbridge for IntelIch9 {
    fn pre_console_init(&mut self) -> Result<(), ServiceError> {
    ...
    fn gpio_get(&self, pin: u32) -> Result<bool, ServiceError> {
        Ok(IchGpio::new(ich9::DEFAULT_GPIOBASE).get(pin as u8))
    }

    fn gpio_set(&self, pin: u32, value: bool) -> Result<(), ServiceError> {
        IchGpio::new(ich9::DEFAULT_GPIOBASE).set(pin as u8, value);
        Ok(())
    }
...
impl fstart_superio::LpcBaseProvider for IntelIch9 {
    fn lpc_base(&self) -> u16 {
        // ICH9-M exposes LPC decode for board-specific SuperIO/EC policy.
        // X200 has no onboard SuperIO UART; this legacy base is only a
        // compatibility default for callers that need an LPC config port.
        0x164e
    }
}

impl SmBus for IntelIch9 {
...
// ---------------------------------------------------------------------------
// ACPI device implementation — ICH9-M southbridge devices
// ---------------------------------------------------------------------------
...
mod acpi_impl {
    ...
    impl AcpiDevice for IntelIch9 {
        type Config = IntelIch9Config;

        /// Produce ICH9-M DSDT content under `\\_SB.PCI0`.
        ///
                ...
                Scope("\\_SB_.PCI0") {
                    ...
                    Device("PCIB") {
                        ...
                        Method("_PRT", 0, NotSerialized) {
                            If (PICM) {
                                // PCI bridge slot swizzle used by ICH9 boards; board-specific
                                // policy should eventually move to config if variants diverge.
                                Return(Package(
                                    Package(0x0000FFFFu32, 0u32, 0u32, 16u32),
                                    Package(0x0000FFFFu32, 1u32, 0u32, 17u32),
                                    Package(0x0000FFFFu32, 2u32, 0u32, 18u32),
                                    Package(0x0000FFFFu32, 3u32, 0u32, 19u32),
                                    Package(0x0001FFFFu32, 0u32, 0u32, 17u32),
                                    Package(0x0001FFFFu32, 1u32, 0u32, 18u32),
                                    Package(0x0001FFFFu32, 2u32, 0u32, 19u32),
                                    Package(0x0001FFFFu32, 3u32, 0u32, 16u32),
                                    Package(0x0002FFFFu32, 0u32, 0u32, 18u32),
                                    Package(0x0002FFFFu32, 1u32, 0u32, 19u32),
                                    Package(0x0002FFFFu32, 2u32, 0u32, 16u32),
                                    Package(0x0002FFFFu32, 3u32, 0u32, 17u32),
                                    Package(0x0003FFFFu32, 0u32, 0u32, 16u32),
                                    Package(0x0003FFFFu32, 1u32, 0u32, 17u32),
                                    Package(0x0003FFFFu32, 2u32, 0u32, 18u32),
                                    Package(0x0003FFFFu32, 3u32, 0u32, 19u32),
                                    Package(0x0004FFFFu32, 0u32, 0u32, 16u32),
                                    Package(0x0004FFFFu32, 1u32, 0u32, 17u32),
                                    Package(0x0004FFFFu32, 2u32, 0u32, 18u32),
                                    Package(0x0004FFFFu32, 3u32, 0u32, 19u32),
                                    Package(0x0005FFFFu32, 0u32, 0u32, 17u32),
                                    Package(0x0005FFFFu32, 1u32, 0u32, 18u32),
                                    Package(0x0005FFFFu32, 2u32, 0u32, 19u32),
                                    Package(0x0005FFFFu32, 3u32, 0u32, 16u32)
                                ));
                            }
                            Return(Package(
                                Package(0x0000FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32), Package(0x0000FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32), Package(0x0000FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0000FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32),
                                Package(0x0001FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32), Package(0x0001FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0001FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32), Package(0x0001FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32),
                                Package(0x0002FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0002FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32), Package(0x0002FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32), Package(0x0002FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32),
                                Package(0x0003FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32), Package(0x0003FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32), Package(0x0003FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0003FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32),
                                Package(0x0004FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32), Package(0x0004FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32), Package(0x0004FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0004FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32),
                                Package(0x0005FFFFu32, 0u32, #{p("\\_SB_.PCI0.LPCB.LNKB")}, 0u32), Package(0x0005FFFFu32, 1u32, #{p("\\_SB_.PCI0.LPCB.LNKC")}, 0u32), Package(0x0005FFFFu32, 2u32, #{p("\\_SB_.PCI0.LPCB.LNKD")}, 0u32), Package(0x0005FFFFu32, 3u32, #{p("\\_SB_.PCI0.LPCB.LNKA")}, 0u32)
                            ));
                        }
                    }
                    ...
                    Device("LPCB") {
                        ...
                        Device("LNKA") {
                            ...
                            Name("_UID", 1u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKB") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 2u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKC") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 3u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKD") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 4u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKE") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 5u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKF") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 6u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKG") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 7u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
                        Device("LNKH") {
                            Name("_HID", EisaId("PNP0C0F"));
                            Name("_UID", 8u32); Method("_STA", 0, NotSerialized) {
                                Return(0x0Bu32);
                            }
                        }
