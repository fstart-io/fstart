//! ICH9/ICH9 SMM southbridge handler.
...
const ICH9_GPE0_STS: u16 = 0x20;

/// SMM handler for ICH9/ICH9-compatible PMBASE layout.
pub struct Ich9SmmHandler<B = NoBoardSmmHandler>(PhantomData<B>);

impl<B: SmmBoardHandler> SmmHandler for Ich9SmmHandler<B> {
    unsafe fn handle(ctx: &mut SmmContext<'_>) {
        ...
        let gpe_status =
            pm.read32(ICH9_GPE0_STS) as u64 | ((pm.read32(ICH9_GPE0_STS + 4) as u64) << 32);
        B::on_gpe(ctx, gpe_status);
        pm.write32(ICH9_GPE0_STS, 0xffff_ffff);
        pm.write32(ICH9_GPE0_STS + 4, 0xffff_ffff);
