...
pub trait SmmOps: Send + Sync {
    ...
    fn install_smm_handlers(&self, info: &SmmInfo, num_cpus: u16);
...
impl FlightStep {
    ...
    const fn noblock(ap: usize, bsp: usize) -> Self {
        Self {
            barrier: AtomicUsize::new(1),
            cpus_entered: AtomicUsize::new(0),
            ap_fn: AtomicUsize::new(ap),
            bsp_fn: AtomicUsize::new(bsp),
        }
...
pub fn mp_init<C: CpuOps>(config: &MpConfig<'_, C>) -> Result<MpHandle, MpError> {
    ...
    if num_aps == 0 {
        // Single-CPU system.  Run cpu_init on BSP and return.
    ...
    FLIGHT_PLAN[step_count]
        .ap_fn
        .store(ap_mailbox_loop as usize, Ordering::Release);
    FLIGHT_PLAN[step_count].bsp_fn.store(0, Ordering::Release);
    step_count += 1;
...
/// Physical page number for the SIPI vector (0x1000 = page 1).
/// Below conventional memory, above the real-mode IVT at page 0.
/// Avoids the 0x30000 SMM default area.
const SIPI_VECTOR_PAGE: u32 = 0x01;
...
fn install_sipi_trampoline(_num_aps: u16, _lapic: &Lapic) -> Result<(), MpError> {
    // In a full implementation, this would:
    // 1. Copy the SIPI trampoline (from global_asm!) to SIPI_VECTOR_ADDR
    // 2. Patch the parameter block: GDT pointer, stack top, CR3, AP entry
    // 3. Mirror BSP's MTRRs into the parameter block
    //
    // The trampoline assembly will be added in sipi.rs.
    // For now, the structure is in place.

    // TODO: implement SIPI trampoline copy + parameter patching
    // See sipi.rs (to be added) for the trampoline assembly.

    fstart_log::info!("mp: SIPI trampoline at {:#x}", SIPI_VECTOR_ADDR);
    Ok(())
}
...
impl MpHandle {
    ...
    pub fn park_aps(&self) {
        ...
        for i in 0..self.num_aps as usize {
            ...
            mb.func.store(park_cpu as usize, Ordering::Release);
        }
...
impl<'scope, 'env> Scope<'scope, 'env> {
        ...
        F: Fn() + Send + Sync + 'env,
    {
        ...
        mb.func
            .store(trampoline_void::<F> as usize, Ordering::Release);
    ...
    fn dispatch_all(&self, trampoline: fn(*const (), u32), data: usize) {
        ...
        for i in 0..self.handle.num_aps as usize {
            let mb = &MAILBOXES[i];
            mb.func.store(trampoline as usize, Ordering::Release);
        }
        self.wait_all();
    }
