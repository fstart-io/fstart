...
pub use fstart_hda::{
    HdaConfig, HdaVerbTable, PinColor, PinConfig, PinConn, PinConnector, PinDevice, PinGeoLoc,
    PinLoc,
};
...
pub enum LpcFixedDecode {
    /// COMA at 0x3f8 and COMB at 0x2f8.
    #[default]
    ComA3f8ComB2f8,
}
...
impl Default for Bd82x6xLpcDecodeConfig {
    fn default() -> Self {
        Self {
            fixed: LpcFixedDecode::ComA3f8ComB2f8,
            gen_dec: [0; 4],
        }
    }
}

/// PIRQ route selector for LPC PIRQ[A-H]_ROUT registers.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
pub enum PchIrqRoute {
    Irq3,
    Irq4,
    Irq5,
    Irq6,
...
/// GPIO route selector for LPC GPIO_ROUT two-bit fields.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
pub enum GpiRoute {
    #[default]
    None,
...
/// Maximum SATA speed advertised by SATA Interface Speed Support.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
pub enum SataSpeed {
    #[default]
    ChipDefault,
...
/// One Cougar Point SATA port policy entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
pub struct Bd82x6xSataPortConfig {
    #[serde(default)]
    pub enabled: bool,
    #[serde(default)]
    pub max_speed: SataSpeed,
}

/// One PCIe root-port policy entry.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
pub struct Bd82x6xPciePortConfig {
    #[serde(default)]
    pub enabled: bool,
    #[serde(default)]
    pub hotplug: bool,
...
/// Semantic VSCC presets for the PCH SPI controller.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum Bd82x6xSpiVscc {
    /// Coreboot X220 value 0x2005 for the attached SPI flash parts.
    X220Flash,
}
...
impl Bd82x6xSpiVscc {
    const fn encode(self) -> u32 {
        match self {
            Self::X220Flash => 0x2005,
        }
    }
}

/// SPI VSCC override policy.
#[derive(Debug, Clone, Copy, Default, Serialize, Deserialize)]
pub struct Bd82x6xSpiConfig {
    #[serde(default)]
    pub upper: Option<Bd82x6xSpiVscc>,
    #[serde(default)]
    pub lower: Option<Bd82x6xSpiVscc>,
}
...
pub struct IntelBd82x6xConfig {
    ...
    pub ecam_base: u64,
    /// PIRQ routing (A..H).
    pub pirq_routing: [PchIrqRoute; 8],
    /// GPE0 enable bits.
    #[serde(default)]
    pub gpe0_en: u32,
    /// GPI routing selectors for GPIO0..15.  Coreboot X220 routes GPI1 and
    /// GPI13 to SCI.
    #[serde(default)]
    ...
    pub smbus_base: u16,
    /// SATA port policy. X220 enables ports 0 (HDD bay), 1 (dock), and 2
    /// (mSATA), all with 6.0 Gb/s support.
    #[serde(default)]
    pub sata_ports: [Bd82x6xSataPortConfig; 6],
    ...
    /// PCIe root port policy for ports 1..8.
    #[serde(default)]
    pub pcie_ports: [Bd82x6xPciePortConfig; 8],
    /// Enable zero-based linear PCIe root port functions.
    #[serde(default)]
    pub pcie_port_coalesce: bool,
    /// SPI VSCC override policy.
    #[serde(default)]
    pub spi: Bd82x6xSpiConfig,
...
impl IntelBd82x6x {
    ...
    fn program_spi_vscc(&self) {
        let Some(upper) = self.config.spi.upper else {
            if let Some(lower) = self.config.spi.lower {
                let rcba = self.rcba();
                rcba.regs().lvscc.set(lower.encode());
                rcba.regs().lvscc.set(lower.encode() | (1 << 23));
            }
            return;
        };
        let rcba = self.rcba();
        rcba.regs().uvscc.set(upper.encode());
        if let Some(lower) = self.config.spi.lower {
            rcba.regs().lvscc.set(lower.encode());
            rcba.regs().lvscc.set(lower.encode() | (1 << 23));
        }
    ...
    fn program_gpi_routing(&self) {
        ...
        for (idx, route) in self.config.gpi_routing.iter().enumerate() {
            gpio_rout |= u32::from(route.encode()) << (idx * 2);
        }
