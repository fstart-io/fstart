...
/// Board-owned A20 DDR3 timing and calibration policy.
///
/// Fixed A20 MMIO addresses are intentionally not configuration.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct A20DramcConfig {
    pub clock: u32,
    pub mbus_clock: u32,
    pub zq: u32,
    pub odt_en: bool,
...
// SAFETY: early boot is single-threaded and accesses fixed MMIO.
unsafe impl Send for A20Dramc {}
// SAFETY: early boot is single-threaded and accesses fixed MMIO.
unsafe impl Sync for A20Dramc {}

impl A20Dramc {
    /// Construct without touching hardware.
    #[must_use]
    pub fn new_from_config(config: &'static A20DramcConfig) -> Self {
        Self {
            // SAFETY: A20 DRAMC and CCU have fixed physical MMIO mappings.
            regs: unsafe { &*(A20_DRAMC_BASE as *const SunxiDramcRegs) },
    ...
    /// Run full DDR3 training and return the detected DRAM size in bytes.
    ...
    /// Return the detected size after successful [`Self::dram_init`].
    #[must_use]
...
impl A20Dramc {
    ...
    fn dramc_init_helper(&self) -> u64 {
        ...
        self.mctl_enable_dll0(self.config.tpr3);
        ...
        if self.config.dqs_gating_delay != 0 {
            self.mctl_set_dqs_gating_delay(0, self.config.dqs_gating_delay);
        }

        // Step 22: Set DQS gating window type
        if self.config.active_windowing {
...
impl A20Dramc {
    ...
    fn write_hpcr(&self, index: usize, value: u32) {
        ...
        unsafe {
            fstart_core::mmio::write32((base + 0x250 + index * 4) as *mut u32, value);
        }
    }
    ...
    fn mctl_setup_dram_clock(&self) {
        ...
        // Setup MBUS clock
        let mbus_clk = if self.config.mbus_clock == 0 {
            300
        } else {
            self.config.mbus_clock
        };
    ...
    fn ddr3_write_recovery(&self) -> u32 {
        let twr_ck = (15 * self.config.clock).div_ceil(1000); // tWR = 15ns for all DDR3
        if twr_ck < 5 {
            1
        } else if twr_ck <= 8 {
            twr_ck - 4
