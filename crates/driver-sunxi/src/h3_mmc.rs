//! Allwinner H3/H2+ MMC0 host controller driver.
//!
...
pub const H3_MMC0_BASE: u64 = 0x01c0_f000;

// ---------------------------------------------------------------------------
// Board policy
// ---------------------------------------------------------------------------

/// H3 MMC controllers supported by this initial boot closure.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub enum H3MmcController {
    /// SD/MMC controller zero.
    Mmc0,
}

/// Board-owned MMC policy. Fixed A20 addresses are not configuration.
#[derive(Debug, Clone, Copy, serde::Serialize, serde::Deserialize)]
#[serde(deny_unknown_fields)]
pub struct H3MmcConfig {
    pub controller: H3MmcController,
}

impl H3MmcConfig {
    /// Construct policy for the supported boot controller.
    #[must_use]
    pub const fn new(controller: H3MmcController) -> Self {
        Self { controller }
    }
...
/// H3 MMC0 host controller.
pub struct H3Mmc {
    regs: &'static SunxiMmcRegs,
    fifo: *mut u32,
    ccu: &'static SunxiH3CcuRegs,
    _config: &'static H3MmcConfig,
    rca: Cell<u16>,
    sdhc: Cell<bool>,
    capacity: Cell<u64>,
}
...
unsafe impl Send for H3Mmc {}
// SAFETY: the controller is a fixed MMIO peripheral used during single-threaded boot.
unsafe impl Sync for H3Mmc {}

impl H3Mmc {
    /// Construct without touching hardware.
    #[must_use]
    pub fn new_from_config(config: &'static H3MmcConfig) -> Self {
        match config.controller {
            H3MmcController::Mmc0 => Self {
                // SAFETY: H3 MMC0 and CCU have fixed physical MMIO mappings.
                regs: unsafe { &*(H3_MMC0_BASE as *const SunxiMmcRegs) },
                fifo: (H3_MMC0_BASE as usize + 0x200) as *mut u32,
...
impl H3Mmc {
    /// MMC0 module clock register.
    fn mmc_clk_reg(&self) -> &MmioReadWrite<u32, MMC_CLK::Register> {
        self.ccu.mmc_clk(0)
    }

    /// Configure PF0-PF5 for the H3 MMC0 function.
    fn setup_gpio(&self) {
    ...
    /// Enable MMC0's AHB gate and initial 24 MHz module clock.
    fn setup_clocks(&self) {
        let gate_addr = (H3_CCU_BASE as usize + CCU_AHB_GATE0_OFF) as *mut u32;
        // SAFETY: AHB gate register at the fixed H3 CCU mapping.
        unsafe {
            let gate = core::ptr::read_volatile(gate_addr);
            core::ptr::write_volatile(gate_addr, gate | (1 << 8));
        }
        // H3 has a separate bus-reset register for MMC0.
        unsafe {
            let reset = (H3_CCU_BASE as usize + 0x2c0) as *mut u32;
            core::ptr::write_volatile(reset, core::ptr::read_volatile(reset) | (1 << 8));
        }
        self.mmc_clk_reg()
    ...
    fn set_mod_clk(&self, target_hz: u32) {
        self.set_mod_clk_legacy(target_hz);
    }
