//! Allwinner A20 MMC0 host controller driver.
//!
...
/// A20 MMC controllers supported by this initial boot closure.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub enum A20MmcController {
...
pub struct A20MmcConfig {
    pub controller: A20MmcController,
}

impl A20MmcConfig {
    /// Construct policy for the supported boot controller.
    #[must_use]
    pub const fn new(controller: A20MmcController) -> Self {
        Self { controller }
    }
...
/// A20 MMC0 host controller.
pub struct A20Mmc {
    regs: &'static SunxiMmcRegs,
    fifo: *mut u32,
    ccu: &'static SunxiA20CcuRegs,
    _config: &'static A20MmcConfig,
    rca: Cell<u16>,
    sdhc: Cell<bool>,
    capacity: Cell<u64>,
}
...
unsafe impl Send for A20Mmc {}
// SAFETY: the controller is a fixed MMIO peripheral used during single-threaded boot.
unsafe impl Sync for A20Mmc {}

impl A20Mmc {
    /// Construct without touching hardware.
    #[must_use]
    pub fn new_from_config(config: &'static A20MmcConfig) -> Self {
        match config.controller {
            A20MmcController::Mmc0 => Self {
                // SAFETY: A20 MMC0 and CCU have fixed physical MMIO mappings.
                regs: unsafe { &*(A20_MMC0_BASE as *const SunxiMmcRegs) },
                fifo: (A20_MMC0_BASE as usize + 0x100) as *mut u32,
...
impl A20Mmc {
    ...
    /// Configure PF0-PF5 for the A20 MMC0 function.
    fn setup_gpio(&self) {
    ...
    fn setup_clocks(&self) {
        let gate_addr = (A20_CCU_BASE as usize + CCU_AHB_GATE0_OFF) as *mut u32;
        // SAFETY: AHB gate register at the fixed A20 CCU mapping.
        unsafe {
