...
use fstart_smm::{
    NoBoardSmmHandler, SMM_RUNTIME_FLAG_FINALIZED, SmmBoardHandler, SmmContext, SmmHandler,
};
...
impl<B: SmmBoardHandler> SmmHandler for Ich8SmmHandler<B> {
    unsafe fn handle(ctx: &mut SmmContext<'_>) {
        unsafe {
            let pm_base =
                ctx.params.platform_data[fstart_smm::SMM_PLATFORM_DATA_ICH_PM_BASE] as u16;
            if pm_base == 0 {
                return;
            }
