...
impl LpcDecodeConfig {
    ...
    pub const fn new() -> Self {
        Self {
            ...
            generic_io: ConstVec::new(LpcGenericIoDecode { base: 0, size: 0 }),
        }
    }
}
...
pub use crate::southbridge::gpio_ich::{
    GpioConfig, GpioDir, GpioLevel, GpioMode, GpioPin, GpioReset, IchGpio,
};
...
impl Default for IntelIch7Config {
    fn default() -> Self {
        Self::new()
    }
}
...
impl IntelIch7 {
    ...
    fn ioapic_read(reg: u32) -> u32 {
        ...
        unsafe {
            fstart_core::mmio::write32(IOAPIC_BASE as *mut u32, reg);
            fstart_core::mmio::read32((IOAPIC_BASE + 0x10) as *const u32)
        }
    }
    ...
    fn ioapic_write(reg: u32, value: u32) {
        ...
        unsafe {
            fstart_core::mmio::write32(IOAPIC_BASE as *mut u32, reg);
            fstart_core::mmio::write32((IOAPIC_BASE + 0x10) as *mut u32, value);
        }
    }
    ...
    fn setup_ioapic(&self) {
        ...
        let bsp_lapic_id =
            unsafe { fstart_core::mmio::read32((LAPIC_BASE + 0x20) as *const u32) >> 24 };
...
impl Southbridge for IntelIch7 {
    fn gpio_get(&self, pin: u32) -> Result<bool, ServiceError> {
        Ok(IchGpio::new(DEFAULT_GPIOBASE as u16).get(pin as u8))
    }

    fn gpio_set(&self, pin: u32, value: bool) -> Result<(), ServiceError> {
        IchGpio::new(DEFAULT_GPIOBASE as u16).set(pin as u8, value);
        Ok(())
    }
}
...
impl IntelIch7 {
    ...
    pub fn ramstage_init(&self) -> Result<(), ServiceError> {
        ...
        lpc.or8(0xA9, (1 << 4) | (1 << 3) | (1 << 2));
        let v = lpc.read8(0xAA);
        lpc.write8(0xAA, (v & 0xF0) | (2 << 2) | 2);
    ...
    fn sata_init(&self, sata: &SataConfig) {
        ...
        sata_dev.write8(0xA6, 0x22);
        sata_dev.write8(0xA0, 0x78);
        sata_dev.write8(0xA6, 0x22);
        sata_dev.write8(0xA0, 0x88);
        let v = sata_dev.read32(0xA4);
        sata_dev.write32(0xA4, (v & 0xC0C0_C0C0) | 0x1B1B_1212);
        sata_dev.write8(0xA0, 0x8C);
        let v = sata_dev.read32(0xA4);
    ...
    fn i8259_init(&self) {
        ...
        unsafe {
            ...
            fstart_core::pio::outb(0x4D1, elcr2 | (1 << 1)); // IRQ 9 = bit 1 of ELCR2
        }
    }
    ...
    fn pcie_init(&self) {
        ...
        for func in 0u8..6 {
            ...
            port.write32(0x114, (vc0 & !0xFF) | 1);

            // Mask completion timeouts.
            port.or32(0x148, 1 << 14);
    ...
    fn hda_init(&self, hda: &HdaConfig) {
        ...
        hda_dev.and8(0x40, !(1 << 3)); // Clear it
        hda_dev.or8(0x40, 1 << 2); // Enable clock detection
...
mod acpi_impl {
    ...
    impl X86PlatformProvider for IntelIch7 {
